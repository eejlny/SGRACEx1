...
class FPYNQ_GAT(torch.autograd.Function):
    ...
    @staticmethod
    def forward(ctx,my_ip, self,adj,nnz_adj,input, weights,weights_linear,weights_l2,attention,out_features,dropout,relu):
        """
        In the forward pass we receive a Tensor containing the input and return
        a Tensor containing the output. ctx is a context object that can be used
        to stash information for backward computation. You can cache arbitrary
        ...
        if (config.acc==1):
         ...
         support_pynq_linear_q = support_pynq_linear_q.reshape(1, (weights_linear.shape[0]*weights_linear.shape[1]))
         ...
         if(config.core_count == 1):
           if(config.instant_layer_count == 1):
             ...
             B2_buffer[0:(weights_linear.shape[0]*weights_linear.shape[1])] = support_pynq_linear_q.astype(config.float_type)
           else:
            if(layern==1):
             B_buffer[0:(weights.shape[0]*weights.shape[1])] = support_pynq_q.astype(config.float_type)
             global weight_shift
             support_pynq_q_l2 = support_pynq_q_l2.reshape(1, (weights.shape[1]*weights.shape[1]))
             weight_shift = (weights.shape[0]*weights.shape[1])
             B_buffer[weight_shift:(weight_shift+weights.shape[1]*weights.shape[1])] = support_pynq_q_l2.astype(config.float_type)
         ...
         if (config.profiling == 2):
          ...
          if(config.core_count == 1):
           if(config.instant_layer_count == 1):
            ...
            if(layern == 1):
             if (config.min_output == 0):
              print('start core 1 to process 2 layers')
             amult =  time.time()
             my_ip.register_map.CTRL.AP_START=1
             kernel_done = my_ip.register_map.CTRL.AP_DONE
             while kernel_done == 0:
             ...
             dmult =  time.time()
             if (config.profiling == 1):   
              print('Accelerator forward kernel 2-layer time: {:.5f}ms'.format(1000/1*(dmult - amult)))
             if (config.min_output == 0):
              print('done core 1') 
            else:  
             if (config.min_output == 0): 
              print('Nothing to do in layer 2')
    ...
    def backward(ctx, grad_output):
        ...
        if (config.accb==1):
         ...
         if(sage==1):
          grad_linear_weights = torch.mm(input_t, grad_output)
         elif(linear==1):
          grad_linear_weights = torch.mm(input_t, grad_output) 
         else:
          grad_linear_weights = torch.zeros(size=(input_t.shape[0],grad_output.shape[1])) 
        ...
        return something, something, something,something, grad_input, grad_weights, grad_linear_weights, something, grad_attention, something, something, something
    ...
    def forward(self, stream,dense, relu,input, edge_index,norm, adj,weights_l2):
        ...
        if(config.instant_layer_count==1):
         ...
         if(layern==1):  
          ...
          if(config.acc==1):
           self.my_ip.register_map.nnz_adj1 = nnz_adj
          output = self.fn(self.my_ip,self,adj,nnz_adj,input, self.weight, weights_l2,self.attention,self.out_features,self.dropout,relu)
         else: # second layer
          nnz_adj = len(norm)
          output = self.fn(self.my_ip,self,adj,nnz_adj,input, self.weight, weights_l2, self.attention,self.out_features,self.dropout,relu)        
...
def init_SGRACE():
 ...
 if (config.acc == 1):
  ...
  B2_buffer = allocate((config.N_adj*config.P_w*config.head_count), dtype=config.float_type)
  B_buffer = allocate((config.N_adj*config.P_w+config.P_w*config.P_w), dtype=config.float_type) #two layers
 ...
 if (config.w_qbits == 8):   
    ...
    internal_quantization =  8 #16 # 0x0000FFFF#bit QTYPE 32
