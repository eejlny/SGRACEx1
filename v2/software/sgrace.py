...
class FPYNQ_GAT(torch.autograd.Function):
    ...
    def forward(ctx,my_ip, self,adj,nnz_adj,input, weights,weights_linear,weights_l2,weights_l3,attention,attention_l2,attention_l3,out_features,dropout,relu):
        ...
        if (config.acc==1):
         ...
         my_ip.register_map.quantized_multiplierl = st.internal_quantizationl
...
# =============================================================================
# Contents:
#   1. SGRACEState          — centralised hardware state (quantisation ranges,
#                             PYNQ buffers, register-map writes).
...
class SGRACEState:
    ...
    variables that were previously scattered across the module namespace.

    Usage
    -----
    Call ``SGRACEState.init(number_of_classes)`` **once** at startup in place
    Call ``SGRACEState.init(number_of_classes)`` once at startup.  Every layer object
    ...
    def init(cls, number_of_classes: int, op_type: str = "GCN",
        ...
        Allocate PYNQ buffers, compute all quantisation constants, and write
        static addresses to the hardware register map.  Returns the singleton
        state object.

        Args:
            number_of_classes: number of output classes.
            op_type:           GNN operator type string (e.g. "GCN", "GAT").
            calib_file:        optional path to a JSON calibration file
                               produced by ``calibrate_sgrace()``.  When
                               supplied, activation ranges are taken from the
                               file instead of the hardcoded defaults.
        """
        st = cls()
        ...
        if config.acc == 1:
            if config.large_config == 1:
             ol = Overlay("gat_all_unsigned_8_8.bit")
            else: 
             ol = Overlay("gat_all_unsigned_1_4.bit")
            
            st.my_ip = ol.mmult_top_0

        # ── quantisation range setup ──────────────────────────────────────────
        st._setup_quant_ranges(calib_file=calib_file)
    ...
    def recalibrate(self, calib_file: str):
        """Reload activation ranges from *calib_file* and recompute all
        quantisation constants in-place.

        Call this after ``calibrate_sgrace()`` has written the JSON file but
        before training begins.  PYNQ buffers and the hardware register map are
        not touched — only the scale/zero-point constants are updated.

        Args:
            calib_file: path to JSON file produced by ``calibrate_sgrace()``.

        Example::

            state = SGRACEState.init(num_classes)       # hardcoded defaults
            model = MyGNN(state, ...)                   # build model
            calibrate_sgrace(model, data,               # collect stats
                             out_file="calib.json")
            state.recalibrate("calib.json")             # update constants
            # train ...
        """
        # ── reload activation ranges ──────────────────────────────────────────
        self._setup_quant_ranges(calib_file=calib_file)

        # ── recompute all quant scale/zero-point constants ────────────────────
        # Weight ranges are unchanged so w_* constants are recomputed for
        # consistency but will produce the same values as before.
        self.w_s_o,  self.w_s,  self.w_z  = generate_quantization_qbits_constants(
            self._w_min,  self._w_max,  config.w_qbits)
        self.w_s_o2, self.w_s2, self.w_z2 = generate_quantization_qbits_constants(
            self._w_min2, self._w_max2, config.w_qbits)
        self.w_s_ol, self.w_sl, self.w_zl = generate_quantization_qbits_constants(
            self._w_minl, self._w_maxl, config.w_qbitsl)
        self.a_s_o,  self.a_s,  self.a_z  = generate_quantization_uqbits_constants(
            self._a_min,  self._a_max,  config.w_qbits)
        self.f_s_o,  self.f_s,  self.f_z  = generate_quantization_uqbits_constants(
            self._f_min,  self._f_max,  config.w_qbits)
        self.f_s_o2, self.f_s2, self.f_z2 = generate_quantization_uqbits_constants(
            self._f_min2, self._f_max2, config.w_qbits)
        self.f_s_ol, self.f_sl, self.f_zl = generate_quantization_uqbits_constants(
            self._f_minl, self._f_maxl, config.w_qbitsl)
        self.l_s_ol, self.l_sl, self.l_zl = generate_quantization_qbits_constants(
            self._l_minl, self._l_maxl, config.w_qbitsl)

        # ── recompute dequantisation factors ──────────────────────────────────
        go_min, go_max = -0.10, 0.10
        self.go_s_o, self.go_s, self.go_z = generate_quantization_uqbits_constants(
            go_min, go_max, 8)
        self.deq_o  = self.w_s_o  * self.f_s_o  * self.a_s_o
        self.deq_o2 = self.w_s_o2 * self.f_s_o2 * self.a_s_o
        self.deq_ol = self.w_s_ol * self.l_s_ol
        self.deq_gw = self.f_s_o  * self.a_s_o  * self.go_s_o
        self.deq_gi = self.a_s_o  * self.go_s_o * self.w_s_o
        self._apply_qbit_deq_corrections()

        # ── update PYNQ buffers with new constants ────────────────────────────
        # These buffers were populated in _allocate_buffers() with the old
        # constants — they must be refreshed so the hardware sees correct values.
        if self.quantization_scale_fea_buffer is not None:
        ...
        if self.quantization_scale_w_buffer is not None:
        ...
        if self.quantization_scale_l_buffer is not None:
        ...
        if self.deq_factor_buffer is not None:
            self.deq_factor_buffer[0] = self.deq_o
            self.deq_factor_buffer[1] = self.deq_o2
            self.deq_factor_buffer[2] = self.deq_ol

        if self.scale_fea_buffer is not None:
            self.scale_fea_buffer[0] = self.scale_fea
            self.scale_fea_buffer[1] = self.scale_fea2
            self.scale_fea_buffer[2] = self.scale_feal

        print("[SGRACEState] recalibrate() complete.")
        print(f"  f  scale : {1/self.f_s:.6f}  zero: {self.f_z}")
        print(f"  f2 scale : {1/self.f_s2:.6f}  zero: {self.f_z2}")
        print(f"  deq_o    : {self.deq_o:.6f}")

    def _setup_quant_ranges(self, calib_file: str = None):
        """Per-bit-width quantisation range lookup.

        Hardcoded defaults are used as a baseline.  If *calib_file* points to
        a JSON file produced by ``calibrate_sgrace()``, any activation ranges
        it contains (keys ``ranges`` and ``ranges_l``) override the defaults.
        Weight ranges (``w``, ``w2``, ``wl``) are never overridden by the
        calibration file because they are determined solely by the model
        weights, not by the data distribution.
        """
        import json, pathlib
        ...
        # Default ranges used when no calibration file is supplied.
        # w/w2/wl use the original per-bit-width clamps derived from offline
        # dataset experiments.  When a calib_file IS supplied, w/w2/wl are
        # widened to [-1, +1] to match the training-time weight clamp.
        # Activation clamps (f, f2, a, fl, ll) are always overridden by the
        # calibration file when present.
        _ranges = {
            8: dict(w=(-0.5, 0.5), w2=(-1.0, 1.0), a=(0.0, 1.0), f=(0.0, 1.0),
                    f2=(0.0, 1.0), scale_fea=3, scale_fea2=3, deq_shift=2),
        ...
        _ranges_l = {
            ...
            8:  dict(wl=(-1.0, 1.0), fl=(0.0, 8.0), ll=(-4.0, 4.0),
                     scale_feal=3, deq_shift_l=1),
            4:  dict(wl=(-0.5, 0.5), fl=(0.0, 0.5), ll=(-1.0, 1.0),
                     scale_feal=2, deq_shift_l=1),
            2:  dict(wl=(-0.5, 0.5), fl=(0.0, 0.01), ll=(-1.0, 1.0),
                     scale_feal=1, deq_shift_l=1),
            1:  dict(wl=(-0.1, 0.1), fl=(0.0, 0.1), ll=(-0.1, 0.1),
                     scale_feal=1, deq_shift_l=1),
        }

        r  = dict(_ranges.get(qb,  _ranges[8]))   # shallow copy so we can mutate
        rl = dict(_ranges_l.get(qbl, _ranges_l[8]))

        # ── override ranges from calibration file ────────────────────────────
        # When calibration is used, weight ranges are widened to [-1, +1] to
        # match the training-time clamp; activation/adjacency ranges come from
        # the measured data statistics.
        if calib_file and pathlib.Path(calib_file).exists():
            cal = json.load(open(calib_file))
            # Widen weight ranges to [-1, +1] for calibrated runs
            r["w"]  = (-1.0, 1.0)
            r["w2"] = (-1.0, 1.0)
            rl["wl"] = (-1.0, 1.0)
            # Override activation and adjacency keys from measured statistics
            for k, v in cal.get("ranges",   {}).items():
                if k in {"f", "f2", "a"}:
                    r[k] = tuple(v)
            for k, v in cal.get("ranges_l", {}).items():
                if k in {"fl", "ll"}:
                    rl[k] = tuple(v)
            print(f"[SGRACEState] Loaded calibration ranges from '{calib_file}'")
        elif calib_file:
            print(f"[SGRACEState] Warning: calib_file '{calib_file}' not found — "
        ...
        elif (config.h_qbits==1):
         _falign = {1: 0}

        if (config.h_qbitsl==8):
         _falignl = {8: 0, 4: 4, 2: 6, 1: 7}
        elif (config.h_qbitsl==4):
         _falignl = {4: 0,2: 2}
        
        _betaqu = {8: 255, 4: 15, 2: 3, 1: 1}
        self._f_align  = _falign.get(qb,  0)
        self._beta_qu  = _betaqu.get(qb,  255)


        self._f_alignl = _falignl.get(qbl, 0)
        self._beta_qul = _betaqu.get(qbl, 255)

        print("f_align is set to ",self._f_align)
        print("f_alignl is set to ",self._f_alignl)
    ...
    def _apply_qbit_deq_corrections(self):
        ...
        _iq = {8: 8, 4: 4, 2: 2, 1: 1} #internal quantization
        _iql = {8: 8, 4: 4, 2: 2, 1: 1} #internal quantization linear
        self.internal_quantization = _iq.get(config.w_qbits, 16)
        self.internal_quantizationl = _iql.get(config.w_qbitsl, 16)
        print("internal quantization graph is:")
        print(self.internal_quantization)
        print("internal quantization linear is:")
        print(self.internal_quantizationl)
    ...
    def _allocate_buffers(self, number_of_classes: int):
        ...
        ip.register_map.quantized_multiplierl = self.internal_quantizationl
...
# ---------------------------------------------------------------------------
# 1b. calibrate_sgrace() — data-driven activation range calibration
# ---------------------------------------------------------------------------

_SGRACE_LAYER_CLASSES = None  # populated lazily after all classes are defined


def calibrate_sgrace(model, forward_fn, calib_fraction=0.15,
                     percentile=99.9, out_file="quant_ranges.json"):
    """Calibrate SGRACE quantisation ranges from data before training.

    Runs a single forward pass via *forward_fn* (no gradients, no weight
    updates) and collects per-layer output statistics.  The resulting
    99.9th-percentile activation ranges are written to *out_file* in a format
    that ``SGRACEState.init(..., calib_file=out_file)`` and
    ``SGRACEState.recalibrate()`` can consume directly.

    Because SGRACE layer ``forward()`` methods take many explicit arguments
    (stream, dense, relu, edge_index, norm, adj, …) rather than a single
    ``data`` object, the caller must supply *forward_fn* — a zero-argument
    callable that performs one full forward pass of the model.  This keeps
    ``calibrate_sgrace`` decoupled from any specific model signature.

    Args:
        model:          the GNN model (already constructed; random-init weights
                        are fine — only the activation *distribution shape*
                        matters for range calibration).
        forward_fn:     a zero-argument callable that runs one forward pass,
                        e.g. ``lambda: model(x, edge_index, norm, adj, ...)``.
        calib_fraction: informational only — log how much of the dataset the
                        caller is using (default 0.15).
        percentile:     upper percentile for the max bound; lower bound uses
                        ``100 - percentile`` (default 99.9).
        out_file:       path to write the JSON calibration file.

    Returns:
        dict with keys ``ranges`` and ``ranges_l`` mirroring the structure
        consumed by ``_setup_quant_ranges()``.

    Example::

        state = SGRACEState.init(num_classes)
        model = MyGNN(state, ...)

        calibrate_sgrace(
            model,
            forward_fn=lambda: model(x, edge_index, norm, adj, ...),
    ...
    # ── layer class registry (resolved lazily) ────────────────────────────────
    global _SGRACE_LAYER_CLASSES
    if _SGRACE_LAYER_CLASSES is None:
        _SGRACE_LAYER_CLASSES = (
            GCNConv_SGRACE, GATConv_SGRACE, SAGEConv_SGRACE,
            SAGEGAT_SGRACE, GINConv_SGRACE, TransformerConv_SGRACE,
            HighOrderGATConv_SGRACE,
        )

    # ── collect feature and adjacency tensors separately via forward hooks ─────
    # feat_collected[i]  = output tensor of the i-th SGRACE layer (features)
    # adj_collected[i]   = norm/adjacency input to the i-th SGRACE layer
    feat_collected = []
    adj_collected  = []
    hooks = []

    def _make_feat_hook(layer_idx):
        """Collect the layer output (feature activations)."""
        def _hook(module, inp, out):
            if isinstance(out, torch.Tensor):
                feat_collected.append(out.detach().float().cpu().flatten())
        return _hook

    def _make_adj_hook(layer_idx):
        """Collect the norm/adjacency input tensors.

        The SGRACE layer forward signature is:
          forward(self, stream, dense, relu, input, edge_index, norm, adj, ...)
                         0      1     2     3       4           5    6
        norm (index 5) contains the normalised edge weights (dense float).
        adj  (index 6) may be a sparse tensor — we extract .values() in that case.
        """
        def _hook(module, args):
            for idx in (5, 6):
                if idx < len(args):
                    ...
                    if t.is_sparse:
                        t = t.coalesce().values()
                    adj_collected.append(t.detach().float().cpu().flatten())
        return _hook

    for i, module in enumerate(m for m in model.modules()
                                if isinstance(m, _SGRACE_LAYER_CLASSES)):
        hooks.append(module.register_forward_hook(_make_feat_hook(i)))
        hooks.append(module.register_forward_pre_hook(_make_adj_hook(i)))

    # ── run calibration forward pass ──────────────────────────────────────────
    print(f"[calibrate_sgrace] Running calibration forward pass "
          f"(~{calib_fraction*100:.0f}% of dataset) ...")
    was_training = model.training
    model.eval()

    # Save SGRACEState mutable fields that the forward pass modifies,
    # so training starts from a clean state after calibration.
    st = SGRACEState.get() if SGRACEState._instance is not None else None
    saved_layern = st.layern if st is not None else None
    ...
    # Restore training mode and SGRACEState
    if was_training:
        model.train()
    if st is not None and saved_layern is not None:
        st.layern = saved_layern

    if not feat_collected:
        raise RuntimeError(
            "[calibrate_sgrace] No activations collected — make sure the model "
            "contains at least one SGRACE layer and the forward pass runs."
        )

    # ── helper: percentile bounds ─────────────────────────────────────────────
    def _pct(tensors):
    ...
    # ── feature ranges: first layer (f) vs deeper layers (f2 / fl) ───────────
    n_layers = len(feat_collected)
    if n_layers >= 2:
        f_lo,  f_hi  = _pct(feat_collected[:1])
        f2_lo, f2_hi = _pct(feat_collected[1:])
    else:
        f_lo,  f_hi  = _pct(feat_collected)
        f2_lo, f2_hi = f_lo, f_hi

    # Clamp to non-negative (unsigned quantisation)
    f_lo,  f_hi  = max(f_lo,  0.0), max(f_hi,  1e-6)
    f2_lo, f2_hi = max(f2_lo, 0.0), max(f2_hi, 1e-6)

    # ── adjacency/norm ranges (a) ─────────────────────────────────────────────
    if adj_collected:
        a_lo, a_hi = _pct(adj_collected)
        a_lo = max(a_lo, 0.0)   # normalised adjacency is always non-negative
        a_hi = max(a_hi, 1e-6)
    else:
        # fallback: normalised adjacency is in (0, 1] by construction
        a_lo, a_hi = 0.0, 1.0
        print("[calibrate_sgrace] Warning: no adjacency tensors captured — "
              "using default a range [0.0, 1.0].")
    ...
    result = {
        ...
        "ranges": {
            "f":  [f_lo,  f_hi],   # first GNN layer feature output
            "f2": [f2_lo, f2_hi],  # deeper GNN layer feature outputs
            "a":  [a_lo,  a_hi],   # adjacency / norm coefficients
            # w, w2 — weights always [-1, +1], never overridden here
        },
        "ranges_l": {
            "fl": [f2_lo, f2_hi],  # linear layer activations
        },
    }
    ...
    print(f"[calibrate_sgrace] Calibration complete.")
    print(f"  f   range : [{f_lo:.4f},  {f_hi:.4f}]")
    print(f"  f2  range : [{f2_lo:.4f},  {f2_hi:.4f}]")
    print(f"  adj range : [{a_lo:.4f},  {a_hi:.4f}]")
